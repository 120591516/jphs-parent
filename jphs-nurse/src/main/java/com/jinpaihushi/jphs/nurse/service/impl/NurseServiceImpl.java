...
import com.jinpaihushi.jphs.nurse.dao.NursePojoDao;
...
public class NurseServiceImpl extends BaseServiceImpl<Nurse> implements NurseService {
    ...
    @Autowired
    private NursePojoDao nursePojoDao;
    ...
    @Override
    public List<Map<String, Object>> getRecommendNurse(Map<String, Object> query) {
        String time = (String) query.get("time");
        String calendar = time.substring(0, 10);
        Integer h = Integer.parseInt(time.substring(11, 13));
        query.put("calendar", calendar);
        query.put("h", "h_" + h);
    ...
    public List<Map<String, Object>> exportExcel(Nurse nurse) {
        return nurseDao.exportNurseExcel(nurse);
    }
     ...
     * @throws IOException 
     */
    public byte[] ifNurseLegitimate(String id, String token) throws IOException {
        User user = new User();
        user.setStatus(1);
        user.setType(0);
        user.setId(id);
        ...
        if (user == null) {
            return JSONUtil.toJSONResult(0, "护士信息为空！", "");
        }
        /*id=10086
        token=14e1b600b1fd579f47433b88e8d85291
        token生成规则：手机号+密码+日期2017-10-25，双重MD5加密组合*/
        Date date = new Date();
        ...
        System.out.println(MD5.md5crypt(MD5.md5crypt(user.getPhone() + user.getPassword() + formatter.format(date))));
        if (!token.equals(MD5.md5crypt(MD5.md5crypt(user.getPhone() + user.getPassword() + formatter.format(date))))) {
            if (!token.equals(
                    MD5.md5crypt(MD5.md5crypt(user.getPhone() + user.getPassword() + formatter1.format(date))))) {
                return JSONUtil.toJSONResult(0, "token验证失败！", "");
            }
        }
        return JSONUtil.toJSONResult(1, "认证成功！", "");
    }

    public static void main(String[] args) throws ParseException {
        String a = CycleTimeUtils.getPastDate(11);
