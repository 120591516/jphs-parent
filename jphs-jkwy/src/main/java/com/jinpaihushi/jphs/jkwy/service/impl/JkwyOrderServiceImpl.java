...
public class JkwyOrderServiceImpl extends BaseServiceImpl<JkwyOrder> implements JkwyOrderService {
    ...
    public JSONObject getHealthyArchives(JkwyOrder jkwyOrder) {
        ...
        List<JkwyOrder> jkwyPackagePriceList = jkwyOrderDao.getHealthyArchivesAndOrder(jkwyOrder);
        ...
        String rs = (String) transactionTemplate.execute(new TransactionCallback<Object>() {
            public String doInTransaction(final TransactionStatus status) {
                try {
                    ...
                    transaction.setOrderId(jkwyOrder_up.getId());
    ...
    public JSONObject createOrder(JkwyOrder jkwyOrder, String jkwyRelationId) {
        ...
        String rs = (String) transactionTemplate.execute(new TransactionCallback<Object>() {
            ...
            public String doInTransaction(final TransactionStatus status) {
                try {
                    User user = userDao.loadById(jkwyOrder.getCreatorId());
                    if (user == null) {
                        if (Util.debugLog.isDebugEnabled()) {
                    ...
                    jkwyOrder.setCreatorName(user.getName());
                    ...
                    if (supportNumber != jkwyRelationId_list.size()) {
                        if (Util.debugLog.isDebugEnabled()) {
                            Util.debugLog.debug("健康优护-下单失败，：创建订单失败，套餐人数=" + supportNumber + "和选择人数="
                                    + jkwyRelationId_list.size() + "不一致");
                        }
                        re_json.put("resultcode", 0);
                        re_json.put("msg",
                                "创建订单失败，套餐人数=" + supportNumber + "和选择人数=" + jkwyRelationId_list.size() + "不一致");
                        re_json.put("result", 0);
                        return re_json.toString();
                    }
                    //	 订单绑定亲友
                    ...
                    payPrice_one = DoubleUtils.round(payPrice_one, 2);
                    ...
                    if (!StringUtils.isEmpty(jkwyOrder.getCode())) {
                        JkwyCode jkwyCode = new JkwyCode();
                        jkwyCode.setCode(jkwyOrder.getCode());
                        jkwyCode.setStatus(0);
                        JkwyCode jkwyCode_one = jkwyCodeDao.load(jkwyCode);
                        if (jkwyCode_one != null) {
                            jkwyOrder.setRecommendId(jkwyCode_one.getRecommendId());
                        }
                    }
                    ...
                    if (DoubleUtils.subToPositive(payPrice_one, 0) == 0) {

                        jkwyOrder.setSchedule(1);
                        jkwyOrder.setPayTime(new Date());
                        jkwyOrder.setPayPrice(0d);
                        jkwyOrderDao.update(jkwyOrder);

                        // 修改亲友订单关系表
                        JkwyOrderRelation jkwyOrderRelation = new JkwyOrderRelation();
                        jkwyOrderRelation.setJkwyOrderId(orderId);
                        jkwyOrderRelation.setStatus(0);
                        List<JkwyOrderRelation> jkwyOrderRelationList = jkwyOrderRelationDao.list(jkwyOrderRelation);
                        if (jkwyOrderRelationList == null || jkwyOrderRelationList.size() < 1) {
                            status.setRollbackOnly();// 回滚
                        ...
                        for (int j = 0; j < jkwyOrderRelationList.size(); j++) {
                            ...
                            jkwyOrderRelation_one.setStatus(1);
                            if (Util.debugLog.isDebugEnabled()) {
                                Util.debugLog.debug("0元直接支付完成：jkwyOrderRelation_one" + j + "=" + jkwyOrderRelation_one);
                            }
                            int jord = jkwyOrderRelationDao.update(jkwyOrderRelation_one);
                            if (jord <= 0) {
                                status.setRollbackOnly();// 回滚
                        ...
                        if (!StringUtils.isEmpty(jkwyOrder.getVoucherUserId())) {
                            if (Util.debugLog.isDebugEnabled()) {
                                Util.debugLog.debug("0元直接支付完成：VoucherUserId=" + jkwyOrder.getVoucherUserId());
                            }
                            VoucherUse voucherUse_re = voucherUseDao.loadById(jkwyOrder.getVoucherUserId());
                            if (Util.debugLog.isDebugEnabled()) {
                                Util.debugLog.debug("0元直接支付完成：voucherUse_re=" + voucherUse_re);
                            }
                            voucherUse_re.setUseTime(new Date());
                            int vud = voucherUseDao.update(voucherUse_re);
                            if (Util.debugLog.isDebugEnabled()) {
                        ...
                        JkwyPackage jkwyPackage = jkwyPackageDao.loadById(jkwyOrder.getJkwyPackageId());
                        JkwyPackagePrice jkwyPackagePrice_s = jkwyPackagePriceDao
                                .loadById(jkwyOrder.getJkwyPackagePriceId());
                        ...
                        transaction.setOperate(3);
                        transaction.setOperateSource(4);
                        transaction.setRemark(jkwyPackage.getTitle() + "-" + jkwyPackagePrice_s.getTitle());
                        transaction.setWithdraw(0);
                        transaction.setPayType(3);
                        transaction.setCreatorId(jkwyOrder.getCreatorId());
                        transaction.setCreateTime(new Date());
                        transaction.setStatus(1);
                        transaction.setType(3);
                        transactionDao.insert(transaction);
    ...
    @Override
    public Page<JkwyOrder> queryList(JkwyOrder jkwyOrder) {
        return jkwyOrderDao.queryList(jkwyOrder);
    }
