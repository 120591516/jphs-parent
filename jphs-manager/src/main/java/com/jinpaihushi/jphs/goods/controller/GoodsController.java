 ...
 */
@Controller
@RequestMapping(name = "服务", path = "/goods")
public class GoodsController extends BaseController<Goods> {
    ...
    public String insert(HttpSession hs, HttpServletRequest req, HttpServletResponse resp, ModelMap modelMap,
            Goods goods, ListPrice priceGrade, ImageType imageType) {
        String creator_id = "";
        String creator_name = "";
        try {
        ...
        if (goods.getId() != null && !goods.getId().equals("")) {
            if (null == goods.getIsGrabSingle()) {
                goods.setIsGrabSingle(0);
            }
            boolean b = goodsService.updateGoods(goods, priceGrade, imageType, creator_id, creator_name);
            if (b == false) {
                // 跳转到错误页
    ...
    @RequestMapping(name = "根据product id获取商品", path = "/getIfProductGoodsList.json")
    @ResponseBody
    public JSONObject getIfProductGoodsList(HttpSession hs, HttpServletRequest req, HttpServletResponse resp,
        ...
        goods.setStatus(1);
        goods.setOrderby("G.create_time DESC");
        List<Map<String, Object>> g = goodsService.getIfProductGoodsList(goods);
        json.put("goods", g);
