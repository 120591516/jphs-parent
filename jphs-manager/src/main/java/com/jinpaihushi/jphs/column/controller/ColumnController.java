...
public class ColumnController extends BaseController<Column> {
    ...
    public String index(HttpSession hs, HttpServletRequest req, HttpServletResponse resp, ModelMap modelMap,
        ...
        startPage(p, n);
        column.setOrderby("C.create_time DESC");
    ...
    public String insert(HttpSession hs, HttpServletRequest req, HttpServletResponse resp, ModelMap modelMap,
        ...
        if(csList.getCsList() != null){
        	List<com.jinpaihushi.jphs.column.model.ColumnService> columnServiceList = csList.getCsList();
        	for(int c=0;c<columnServiceList.size();c++){
        		com.jinpaihushi.jphs.column.model.ColumnService columnServiceOne = columnServiceList.get(c);
            	if(columnServiceOne.getId() != null && !columnServiceOne.getId().equals("")){
            		columnServiceService.update(columnServiceOne);
            	}else{
            		columnServiceOne.setCreatorId(session_user.getId());
            		columnServiceOne.setCreatorName(session_user.getName());
            		columnServiceOne.setId(UUIDUtils.getId());
            		columnServiceOne.setColumnId(column.getId());
            		columnServiceOne.setCreateTime(new Date());
            		columnServiceService.insert(columnServiceOne);
            	}
        	}
        }
        /*
    ...
    @RequestMapping(name = "选择服务", path = "/selectListProduct.jhtml")
    public String selectListProduct(ModelMap modelMap, String numberLen, String type,String link) {
    	net.sf.json.JSONObject arr = columnServiceService.selectListProduct(type,link);
    	modelMap.put("type", type);
    	modelMap.put("numberLen", numberLen);
    	modelMap.put("link", link);
    	if(!StringUtils.isEmpty(link)){
    		modelMap.put("p", arr.getString("p"));
    	}else{
    		modelMap.put("p", 0);
    	}
    	modelMap.put("linkArr", arr.getJSONArray("goodsList"));
        return "activity/column/selectListProduct";
    }
