...
import com.jinpaihushi.utils.OperateLogUtils;
...
public class ProductController extends BaseController<Product> {
    ...
    @RequestMapping(name = "获取所有品类", path = "/getProductAllList.json")
    @ResponseBody
    public JSONObject getProductAllList(HttpSession hs, HttpServletRequest req, HttpServletResponse resp, ModelMap modelMap,
            Product product) {
    	JSONObject json = new JSONObject();
    	product.setStatus(1);
    	product.setOrderby("P.create_time DESC");
        List<Map<String,Object>> p = productService.getProductAllList(product);
        json.put("product", new JSONArray().fromObject(p));
