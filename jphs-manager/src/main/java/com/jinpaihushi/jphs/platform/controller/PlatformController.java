...
public class PlatformController extends BaseController<Platform> {
    ...
    public String getGoodsTree(HttpSession hs, HttpServletRequest req, HttpServletResponse resp, ModelMap modelMap, Platform platform) {
        JSONObject message = new JSONObject();
        List<TreeNode> list = platformService.getGoodsList(platform);
        message.put("treeData", list);
        modelMap.put("data", message);
    ...
    @RequestMapping(name = "获取所有平台", path = "/getPlatformAllList.json")
    @ResponseBody
    public JSONObject getPlatformAllList(HttpSession hs, HttpServletRequest req, HttpServletResponse resp, Platform platform) {
    	JSONObject json = new JSONObject();
    	platform.setStatus(1);
    	platform.setOrderby("P.create_time DESC");
		List<Map<String,Object>> p = platformService.getPlatformAllList(platform);
		json.put("platform", p);
		return json;
    }
