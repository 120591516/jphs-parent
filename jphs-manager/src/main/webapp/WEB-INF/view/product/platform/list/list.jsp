			...
			<c:when test="${fn:length(list) >0}">
				<c:forEach items="${list}" var="e" varStatus="s">
					<tr class="bg_list_body">
						...
						<td style="text-align:left"><c:out value="${e.company}" /></td>
						<td style="text-align:left"><c:if test="${e.channel==1}">微信</c:if> <c:if
								test="${e.channel==2}">公众号</c:if> <c:if test="${e.channel==3}">APP</c:if>
							<c:if test="${e.channel==4}">网站</c:if></td>
						<td style="text-align:left"><c:out value="${e.contactsName}" /></td>
