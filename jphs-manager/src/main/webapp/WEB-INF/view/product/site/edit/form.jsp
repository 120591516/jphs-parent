...
<form class="form-horizontal">
	<fieldset>
		...
		<input type="hidden" value='<c:if test="${site.status !=null }">${site.status}</c:if><c:if test="${site.status ==null }">0</c:if>' name="status" />
		...
		<div class="marage_right_content"  style="margin-top:0;padding-top:0">
			<table id="dateTable" class="data_table" style="width:925px">
				<tbody>
					<tr >
						<td width="50px">序号</td>
							...
							<c:forEach var="goodsOne" items="${productOne.goodsList }" varStatus="goodsStatus">
								<c:forEach var="priceOne" items="${goodsOne.priceList }" varStatus="priceStatus">
									<tr class="price${productStatus.index }">
										...
										<td><input style="width:100%"  readonly="readonly" name="productList[${productStatus.index }].goodsList[${goodsStatus.index }].priceList[${priceStatus.index }].pricePart.costPrice"  value="${priceOne.pricePart.costPrice }" /></td>
										<td><input style="width:100%"  name="productList[${productStatus.index }].goodsList[${goodsStatus.index }].priceList[${priceStatus.index }].pricePart.price"  value="${priceOne.pricePart.price }" 
										...
										<td><input style="width:100%"  name="productList[${productStatus.index }].goodsList[${goodsStatus.index }].priceList[${priceStatus.index }].pricePart.oldPrice"  value="${priceOne.pricePart.oldPrice }" 
										...
										<td><input style="width:100%"  name="productList[${productStatus.index }].goodsList[${goodsStatus.index }].priceList[${priceStatus.index }].pricePart.profit" value="${priceOne.pricePart.profit }" 
										...
										<td><input style="width:100%"  name="productList[${productStatus.index }].goodsList[${goodsStatus.index }].priceList[${priceStatus.index }].pricePart.maxPrice" value="${priceOne.pricePart.maxPrice }"  onchange="if( ! /^([1-9]\d{0,9}|0)([.]?|(\.\d{1,2})?)$/.test(this.value)){alert('只能输入数字，小数点后只能保留两位');" /></td>
										<td><input style="width:100%"   name="productList[${productStatus.index }].goodsList[${goodsStatus.index }].priceList[${priceStatus.index }].pricePart.outRatio" value="${priceOne.pricePart.outRatio }" 
