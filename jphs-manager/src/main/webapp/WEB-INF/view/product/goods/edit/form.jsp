...
<form class="form-horizontal edit_form">
	<fieldset>
		...
		<input type="hidden" value="<c:if test="${goods.status !=null }">${goods.status}</c:if><c:if test="${goods.status ==null }">0</c:if>" name="status" />
		...
		<div class="form-group">
			...
			<div class="controls col-md-6">
				<!-- 单行单选项目 -->
				<label class="radio radio-inline">
					<input type="radio" value="1" onclick="allowRelease()" name="type" <c:if test="${goods.type == 1}">checked="checked"</c:if>>
					公开
				</label>
				<label class="radio radio-inline">
					<input type="radio" value="0" onclick="vetoRelease()" name="type" <c:if test="${goods.type == 0}">checked="checked"</c:if>>
					内部
				</label>
			</div>
		</div>
		<div class="form-group">
			<label class="control-label col-md-3">是否允许发布：</label>
			<div class="controls col-md-6">
				<!-- 单行单选项目 -->
				<label class="radio radio-inline">
					<input type="radio" value="1" id="isRelease_true" name="isRelease" <c:if test="${goods.isRelease == 1}">checked="checked"</c:if>>
					允许
				</label>
				<label class="radio radio-inline">
					<input type="radio" value="2" id="isRelease_false" name="isRelease" <c:if test="${goods.isRelease == 2}">checked="checked"</c:if>>
					不允许
				</label>
			</div>
		</div>
		<div class="form-group" id="isGrabSingle" style="display: none;">
			<label class="control-label col-md-3">抢单类型：</label>
			<div class="controls col-md-6">
				<!-- 单行单选项目 -->
				<label class="radio radio-inline">
					<input type="radio" value="1" name="isGrabSingle" <c:if test="${goods.isGrabSingle == 1}">checked="checked"</c:if>>
					抢单
				</label>
				<label class="radio radio-inline">
					<input type="radio" value="2" name="isGrabSingle" <c:if test="${goods.isGrabSingle == 2}">checked="checked"</c:if>>
					指定
				</label>
				<label class="radio radio-inline">
					<input type="radio" value="3" name="isGrabSingle" <c:if test="${goods.isGrabSingle == 3}">checked="checked"</c:if>>
					指定+抢单
				</label>
			</div>
		</div>
		...
		<div class="form-group">
			...
			<!-- 文件上传 -->
			<div class="controls col-md-6">
				<img alt="" height="200" width="200" id="moveurls" src="${wap_image.url}" /><div>尺寸  140*140 比例（1：1）<br>*&nbsp;图片格式必须为.png格式</div>
		...
		<div class="form-group">
			...
			<!-- 文件上传 -->
			<div class="controls col-md-6">
				<img alt="" height="200" width="200" id="pcurls" src="${pc_image.url}" /><div>尺寸 360*350 <br>*&nbsp;图片格式必须为.png格式 </div>
		...
		<c:choose>
			...
			<div class="norm_service" <%-- style="display: ${goods.gradeType == 0 || goods.gradeType == null ?'block':'none'};" --%> >
				<c:forEach var="priceOne_g" items="${goods.priceGrade }" varStatus="status_g">
					...
					<div style="margin-top:20px;margin-left:88px;margin-bottom:20px">等级名称：<input style="width: 200px;" type="text" id="gradeName${status_g.index }" value="${priceOne_g.gradeName }" name="price[${status_g.index }].gradeName" /></div>
						<div class="marage_right_content"  style="margin-top:0;padding-top:0">
							<table id="dateTable"  class="data_table  text-center" style="width:800px;margin:0 auto">
								<tbody id="addtrprice${status_g.index }">
									...
									<c:forEach var="priceOne" items="${priceOne_g.price }" varStatus="status">
											...
											<tr class="addClassprice" id="delete${status_g.index }${status.index }">
												...
												<%-- <td><input style="width:100%" class="form-control" id="price${status_g.index }${status.index }" name="priceGrade[${status_g.index }].price[${status.index }].maxPrice"  value="${priceOne.pricePart.maxPrice }"  /></td> --%>
							...
							<div class="btn_add" onclick="addTd(${status_g.index });" style="width:800px">+</div>
							</div>
						</div>
						</c:forEach>
			</div>
	...
	<!-- 		<div class="form-group">
			...
			<label class="control-label col-md-3" for="input01">服务名称：</label>
			<div class="controls col-md-6">
				<input type="text" id="priceGrade0" name="priceGrade[0].gradeName"  class="form-control"  />
			</div>
			<input type="hidden" id="grade0" name="priceGrade[0].grade" value="0" />	
		</div> -->
				...
				<div class="marage_right_content"  style="margin-top:0;padding-top:0">
					...
					<div class="btn_add" onclick="addTd(0);" style="width:800px">+</div>
					</div>
				</div>
			</div>
			</c:otherwise>
