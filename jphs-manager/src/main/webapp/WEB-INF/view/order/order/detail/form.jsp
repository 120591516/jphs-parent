...
<div class="details_box clearfix">
	...
	<div class="col-md-6" style="border-right: 1px solid #e0e0e0;">
		...
		<p>
			<span>服务名称：</span><span class="title_value">${order.orderGoods.title }</span>
		</p>
		<p>
		...
		<p>
			<span>服务医院：</span><span class="title_value">${order.orderOther.hospital }</span>
		</p>
		<p>
		...
		<p>
			<span>推广码：</span><span class="title_value">${order.code }</span>
		</p>
		<p>
...
<div class="details_box clearfix">
<p class="details_box_xinxi">就医证明</p>
	<div class="col-md-6">
		<p class="sunbox">
			<c:choose>
				<c:when test="${fn:length(serviceImagesList) >0}">
					<c:forEach items="${serviceImagesList}" var="e" varStatus="s">
...
<div class="details_box clearfix">
	...
	<div class="col-md-12">
		<p>
			...
			<span>优惠金额：</span>
					<span class="title_value"><c:if test="${order.orderGoods.price - order.orderGoods.payPrice <=0 }">未使用优惠券</c:if>
					<c:if test="${order.orderGoods.price - order.orderGoods.payPrice >0 }"><fmt:formatNumber type="currency" value="${order.orderGoods.price - order.orderGoods.payPrice }" /></c:if></span>
		</p>
		<p>
			<c:choose>
		...
		<c:choose>
			<c:when test="${null!=voucherUse }">
				<p>
					<span>优惠券类型：</span>
					<c:if test="${voucherUse.type==1 }">现金券</c:if>
					<c:if test="${voucherUse.type==2 }">满减卷</c:if>
					<c:if test="${voucherUse.type==3 }">折扣卷</c:if>
					<span>优惠券面额：</span>
