			...
			<c:when test="${fn:length(list) >0}">
				<c:forEach items="${list}" var="e" varStatus="s">
					<tr class="bg_list_body">
						...
						<td>
							<c:out value="${e.user.name}" /><p>
							<c:out value="${e.user.phone}" />
						</td>
						<td>
							<fmt:formatDate var="str" value="${e.workYears}"
						...
						<td><c:if test="${e.status==1 }">已认证</c:if>
						<c:if test="${e.status==0 }">未认证</c:if>
						</td>
