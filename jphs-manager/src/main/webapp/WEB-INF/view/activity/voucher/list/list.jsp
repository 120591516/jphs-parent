				...
				<c:when test="${fn:length(list) >0}">
					<c:forEach items="${list}" var="e" varStatus="s">
						<tr class="bg_list_body">
							...
							<td style="text-align:left" title="${fn:length(e.productName)>0?e.productName:e.goodsName }"> 
							...
							<td> 
								...
								<c:if test="${e.type ==1 }"><fmt:formatNumber value="${e.amount }" type="currency"/></c:if>
								<c:if test="${e.type ==2 }"><fmt:formatNumber value="${e.amount }" type="currency"/></c:if>
								<c:if test="${e.type ==3 }"> ${e.amount*10 }折</c:if>
							 </td>
