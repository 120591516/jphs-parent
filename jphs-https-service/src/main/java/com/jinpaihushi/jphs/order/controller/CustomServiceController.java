...
public class CustomServiceController {
    ...
    @ResponseBody
    public byte[] createService(HttpSession hs, HttpServletRequest req, HttpServletResponse resp,
        ...
        try {
            ...
            if (StringUtils.isEmpty(customService.getSource())||StringUtils.isEmpty(customService.getCreatorId())||StringUtils.isEmpty(customService.getRemark())) {
            	return JSONUtil.toJSONResult(0, "请核对信息后再提交", null);
            }
            User  user = userService.loadById(customService.getCreatorId());
            customService.setName(user.getName());
            customService.setPhone(user.getPhone());
