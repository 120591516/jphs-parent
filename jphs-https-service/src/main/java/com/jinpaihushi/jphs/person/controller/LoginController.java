...
public class LoginController {
    ...
    public byte[] quickLogin(HttpSession hs, HttpServletRequest req, HttpServletResponse resp, String validateCode,
            String smsCode, String phone, String type, String openid, String name, Integer device) {
        try {
            // 记录日志-debug
            if (Util.debugLog.isDebugEnabled()) {
                Util.debugLog.debug("login.quickLogin.json,phone=" + phone + " type=" + type + " smsCode=" + smsCode
                        + " verifyCode=" + validateCode + " device=" + device);
            }
            ...
            // 查空
            if (StringUtils.isEmpty(phone) || StringUtils.isEmpty(type) || device == null
            ...
            if (user == null) {
                ...
                img.setSourceId(user.getId());
                img.setUrl("https://resource.goldnurse.com/images/default_head/yonghu_nan.jpg");
