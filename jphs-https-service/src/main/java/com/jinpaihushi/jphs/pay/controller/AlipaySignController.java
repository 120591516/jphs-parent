...
public class AlipaySignController {
    ...
    @ResponseBody
    public byte[] alipaySign(HttpSession hs, HttpServletRequest req, HttpServletResponse resp, String content,
            String registerDevice, String endType) {
    ...
    @Transactional
    public byte[] alipayNotify(HttpSession hs, HttpServletRequest request, HttpServletResponse response)
            throws IOException {
        //获取支付宝POST过来反馈信息
            ...
            if (trade_status.equals("TRADE_FINISHED")) {
                ...
                try {
                    ...
                    if (orders != null && !"".equals(orders)) {
                        ...
                        // 记录日志-debug
                        if (Util.debugLog.isDebugEnabled()) {
                            Util.debugLog.debug("alipay.notify.json;根据支付宝返回no=" + no + "平台内部订单号查询订单结果：orders="
                                    + JSONObject.fromObject(orders).toString());
                        }
                        ...
                        if (orderGoods_ny != null && !"".equals(orderGoods_ny)) {
                            ...
                            try {
                                ...
                                if (total_fee_ali == payPrice) {
                                    ...
                                    // 记录日志-debug
                                    if (Util.debugLog.isDebugEnabled()) {
                                        Util.debugLog.debug("alipay.notify.json;transaction="
                                        ...
                                        try {
                                            ...
                                            if (orderUser != null) {
                                                ...
                                                nurseJPushService.jpushAlias(
                                                        "您有一笔新的订单待处理，请登录APP查看订单状态并及时联系客户，服务开始时做好录音准备，避免造成纠纷。",
                                    ...
                                    // 记录日志-debug
                                    if (Util.debugLog.isDebugEnabled()) {
                                        Util.debugLog.debug("alipay.notify.json;下单失败！total_fee_ali=" + total_fee_ali
                                                + ";payPrice=" + payPrice);
                                    }
                                }
                            }
                            ...
                            // 记录日志-debug
                            if (Util.debugLog.isDebugEnabled()) {
                                Util.debugLog.debug("alipay.notify.json;下单失败！查询订单goods信息orderGoods_ny="
                                        + JSONObject.fromObject(orderGoods_ny).toString());
                            }
                            System.out.println("下单失败！"); //请不要修改或删除
                        }
    ...
    @SuppressWarnings("rawtypes")
    @RequestMapping(path = "/otherNotifyGoods.json", name = "阿里支付异步回调-商品")
    @Transactional
    public byte[] alipayNotifyGoods(HttpSession hs, HttpServletRequest request, HttpServletResponse response)
            throws IOException {
        //获取支付宝POST过来反馈信息
        Map<String, String> params = new HashMap<String, String>();
        Map requestParams = request.getParameterMap();
        ...
        // 记录日志-debug
        if (Util.debugLog.isDebugEnabled()) {
            Util.debugLog.debug("alipay.otherNotifyGoods.json;支付宝回调返回数据：params=" + params.toString());
        }
        ...
        // 记录日志-debug
        if (Util.debugLog.isDebugEnabled()) {
            Util.debugLog.debug("alipay.otherNotifyGoods.json;支付宝回调返回数据：交易状态trade_status=" + params);
            Util.debugLog.debug("alipay.otherNotifyGoods.json;支付宝回调返回数据：本平台内部订单号no=" + no);
            Util.debugLog.debug("alipay.otherNotifyGoods.json;支付宝回调返回数据：支付宝交易号trade_no=" + trade_no);
            Util.debugLog.debug("alipay.otherNotifyGoods.json;支付宝回调返回数据：支付金额total_fee=" + total_fee);
        }
        //获取支付宝的通知返回参数，可参考技术文档中页面跳转同步通知参数列表(以上仅供参考)//
        if (AlipayNotify.verify(params)) {//验证成功
            //////////////////////////////////////////////////////////////////////////////////////////
            ...
            if (trade_status.equals("TRADE_FINISHED")) {
                ...
                // 记录日志-debug
                if (Util.debugLog.isDebugEnabled()) {
                    Util.debugLog.debug("alipay.otherNotifyGoods.json;支付宝回调返回数据：trade_status：TRADE_FINISHED");
                }
                //注意：
                //该种交易状态只在两种情况下出现
                //1、开通了普通即时到账，买家付款成功后。trade_close 
                ...
                try {
                    ...
                    commodityOrder.setOrderNo(no);
                    commodityOrder.setStatus(1);
                    commodityOrder.setSchedule(0);
                    List<HashMap<String, Object>> commodityOrders_list = commodityOrderService.loadS(commodityOrder);

                    if (commodityOrders_list != null && !"".equals(commodityOrders_list)
                            && commodityOrders_list.size() > 0) {
                        // 记录日志-debug
                        if (Util.debugLog.isDebugEnabled()) {
                            Util.debugLog.debug("alipay.otherNotifyGoods.json;根据支付宝返回no=" + no + "平台内部订单号查询订单结果：orders="
                                    + JSONArray.fromObject(commodityOrders_list).toString());
                        }
                        String userId = "";
                        for (int a = 0; a < commodityOrders_list.size(); a++) {
                            ...
                            // 记录日志-debug
                            if (Util.debugLog.isDebugEnabled()) {
                                Util.debugLog.debug("alipay.otherNotifyGoods.json;" + a + "-map_c="
                            ...
                            // 记录日志-debug
                            if (Util.debugLog.isDebugEnabled()) {
                                Util.debugLog.debug("alipay.otherNotifyGoods.json;transaction="
                                        + JSONObject.fromObject(transaction).toString());
                            }
                        ...
                        try {
                            if (userId != null && !userId.equals("")) {
                                ...
                                if (orderUser != null) {
                                    // 记录日志-debug
                                    if (Util.debugLog.isDebugEnabled()) {
                                        Util.debugLog.debug("alipay.otherNotifyGoods.json;订单用户信息orderUser="
                                                + JSONObject.fromObject(orderUser).toString());
                                    }
                                    // 发送验证码
                                    /*DoPostSms.sendSms("13581912414", "【金牌护士】您的您的订单：" + no + "下单成功。",
                                            "SMS_69155344", "{\"out_trade_no\":\"" + no + "\"}");*/
                                }
                            }
                            response.getWriter().write("success");
                        }
                        ...
                        if (Util.debugLog.isDebugEnabled()) {
                            Util.debugLog
                                    .debug("alipay.otherNotifyGoods.json;下单失败！查询不到该订单数据-查看status状态以及schedule字段是否正确");
                        }
                        System.out.println("下单失败！查询不到该订单数据"); //请不要修改或删除
                    }
                    ...
                    // 记录日志-debug
                    if (Util.debugLog.isDebugEnabled()) {
                        Util.debugLog.debug("alipay.otherNotifyGoods.json;下单失败！异常" + e, e);
                    }
                }
            }
            else {
                // 记录日志-debug
                if (Util.debugLog.isDebugEnabled()) {
                    Util.debugLog.debug("alipay.otherNotifyGoods.json;下单失败！异常-trade_status=" + trade_status);
                }
                response.getWriter().write("success");
            }
            //——请根据您的业务逻辑来编写程序（以上代码仅作参考）——
            //////////////////////////////////////////////////////////////////////////////////////////
                  ...
                  // 记录日志-debug
            if (Util.debugLog.isDebugEnabled()) {
                Util.debugLog.debug("alipay.otherNotifyGoods.json;验证失败！异常-params=" + params.toString());
            }
            System.out.println("fail");
            response.getWriter().write("fail");
        }
    ...
    @SuppressWarnings("rawtypes")
    @RequestMapping(path = "/alipayNotifyJkwy.json", name = "阿里支付异步回调-jkwy")
    @Transactional
    public byte[] alipayNotifyJkwy(HttpSession hs, HttpServletRequest request, HttpServletResponse response)
            throws IOException {
        //获取支付宝POST过来反馈信息
        Map<String, String> params = new HashMap<String, String>();
        Map requestParams = request.getParameterMap();
        ...
        if (AlipayNotify.verify(params)) {//验证成功
            ...
            if (trade_status.equals("TRADE_FINISHED")) {
                ...
                try {
                	...
                	boolean b= jkwyOrderService.jkwyNotify(1, no, trade_no, total_fee, null);
                	if(b){
                		response.getWriter().write("success");
                	}
    ...
    @ResponseBody
    public void withNotify(HttpSession hs, HttpServletRequest request, HttpServletResponse response)
            throws IOException {
        //获取支付宝GET过来反馈信息
        Map<String, String> params = new HashMap<String, String>();
        Map requestParams = request.getParameterMap();
        ...
        // 记录日志-debug
        if (Util.debugLog.isDebugEnabled()) {
            Util.debugLog.debug("alipay.withNotify.json；-out_trade_no=" + out_trade_no + ";trade_no=" + trade_no
