...
public class FileUploadController {
     ...
     */
    @RequestMapping(value = "/aindex.jhtml", method = RequestMethod.POST)
    @ResponseBody
    public byte[] uploads(@RequestParam("file") MultipartFile[] file,
            HttpServletRequest request, HttpServletResponse response) throws IOException {
        JSONObject message = new JSONObject();
        // 可以在上传文件的同时接收其它参数
        System.out.println("文件myfiles=" + file);
        String rerealPath ="E:/Desktop/update/";
        ...
        if(os.toLowerCase().startsWith("win")){  
        	...
        	realPath = "https://resource.goldnurse.com/images/certificate/";
        	rerealPath="/data/resoucre/images/certificate/";
        } 
        // 设置响应给前台内容的数据格式
        ...
        for (MultipartFile myfile : file) {
            if (myfile.isEmpty()) {
                ...
                System.out.println("========================================");
                try {
                	String imageName =  getDateFormat();
                	String imageType =  ".png";
                    // 这里不必处理IO流关闭的问题,因为FileUtils.copyInputStreamToFile()方法内部会自动把用到的IO流关掉
                    // 此处也可以使用Spring提供的MultipartFile.transferTo(File
                    // dest)方法实现文件的上传
                    FileUtils.copyInputStreamToFile(myfile.getInputStream(), new File(rerealPath, imageName+imageType));
                    
                    //					return message;
                    return JSONUtil.toJSONResult(1, "上传成功",realPath+imageName+imageType);
