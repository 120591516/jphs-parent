...
public class CommodityController extends BaseController<Commodity> {
    ...
    @ResponseBody
    public byte[] getShopList(String columnId, String nurseId, Integer p, String sort) {
        System.out.println(">>>>>>>>>>>>>>>>>>>>>>>");
    ...
    @ResponseBody
    public byte[] getShopDetail(HttpSession hs, HttpServletRequest req, HttpServletResponse resp, ModelMap modelMap,
            String commodityId, String columnId, Integer p, String re, String siteId) {
        try {
            if (Util.debugLog.isDebugEnabled()) {
                Util.debugLog
                        .debug("commodity.getShopDetail.json,commodityId =" + commodityId + ",columnId =" + columnId);
            }
            if (StringUtils.isEmpty(commodityId)) {
                return JSONUtil.toJSONResult(0, "参数不能为空", null);
            }
            Commodity commodity = commodityService.getCommodityDetail(columnId, commodityId, siteId);
            ...
            Util.failLog.error("commodity.getShopDetail.json,commodityId =" + commodityId + ",columnId = " + columnId,
                    e);
        }
        return null;
    }
    ...
    @ResponseBody
    public byte[] getSaleByNurse(HttpSession hs, HttpServletRequest req, HttpServletResponse resp, ModelMap modelMap,
    ...
    @ResponseBody
    public byte[] getNurseSale(HttpSession hs, HttpServletRequest req, HttpServletResponse resp, ModelMap modelMap,
            String nurseId, String commodityId, String schedule, Integer p) {
        try {
            if (Util.debugLog.isDebugEnabled()) {
                Util.debugLog.debug("commodity.getNurseSale.json,nurseId =" + nurseId + ",commodityId=" + commodityId
                        + ",schedule =" + schedule);
            }
            if (StringUtils.isEmpty(nurseId)) {
                return JSONUtil.toJSONResult(0, "参数不能为空", null);
            ...
            Util.failLog.error("commodity.getNurseSale.json,nurseId =" + nurseId + ",commodityId=" + commodityId
                    + ",schedule=" + schedule, e);
        }
        return null;
    }
    ...
    @ResponseBody
    public byte[] getListByCar(HttpSession hs, HttpServletRequest req, HttpServletResponse resp,String ids,String siteId,String platformId,String userId) {

        try {
            if (Util.debugLog.isDebugEnabled()) {
                Util.debugLog.debug("commodity.getListByCar.json,ids =" + ids);
            }
            if (StringUtils.isEmpty(ids) || StringUtils.isEmpty(siteId) || StringUtils.isEmpty(platformId) || StringUtils.isEmpty(userId)) {
                return JSONUtil.toJSONResult(0, "参数不能为空", null);
            }

            List<Business> list = commodityService.getListByCar(ids,siteId,platformId,userId);
    ...
    @ResponseBody
    public byte[] getOneDetail(HttpSession hs, HttpServletRequest req, HttpServletResponse resp, ModelMap modelMap,
