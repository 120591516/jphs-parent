...
public class TransactionServiceImpl extends BaseServiceImpl<Transaction> implements TransactionService {
                ...
                try {
                    ...
                    if (transaction.getPayType() == 1) {
                        ...
                        if (map != null) {
                            ...
                            if (StringUtils.isEmpty(map.get("is_success").toString())) {
                                status.setRollbackOnly();// 回滚
                                return "0";
                            }
                        }
     ...
     * 	提现订单记录列表
     */
    public Page<Transaction> withdrawCashTransactionDetail(Transaction transaction){
    	return transactionDao.withdrawCashTransactionDetail(transaction);
    }
