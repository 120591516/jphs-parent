...
public  class AlipaySign {
	...
	public static void main(String[] args) {
		...
		JSONObject sParaTemp = new JSONObject();
			...
			sParaTemp.put("out_trade_no", "JP2017189369457w");
		...
		try{
			...
			if(PRIVATE_KEY != null && !"".equals(PRIVATE_KEY)){
				...
				try {
					...
					if(end_type == 1 || end_type == 2){
						String appcontent = "_input_charset="+content_json.getString("_input_charset");	//	编码格式：utf-8
															appcontent += "&body=" +  content_json.getString("body") ;				//	商品信息
															appcontent += "&notify_url="+content_json.getString("notify_url");		//	异步回调地址
															appcontent += "&out_trade_no=" + content_json.getString("out_trade_no");//	订单号
															appcontent += "&partner=" + PARTNER;									//	商户收款账号
															appcontent += "&payment_type="+content_json.getString("payment_type");	//	支付类型1 ，无需修改
															appcontent += "&return_url="+content_json.getString("return_url");		//	同步回调
															appcontent += "&seller_id=" + SELLER_ID;								//	收款支付宝账号，以2088开头由16位纯数字组成的字符串，一般情况下收款账号就是签约账号
															appcontent += "&service="+ALI_SERVICE_URL;								//	生成签名支付宝url地址
															appcontent += "&show_url="+content_json.getString("show_url");			//	商品展示页面
															appcontent += "&subject=" + content_json.getString("subject") ;			//	商品名称
															appcontent += "&total_fee="  + content_json.getString("total_fee");		//	金额
															appcontent += "&sign=" + sign;											//	金额
															appcontent += "sign_type=RSA";
						
						jsonSign.put("sign", sign);
						jsonSign.put("content", appcontent);
