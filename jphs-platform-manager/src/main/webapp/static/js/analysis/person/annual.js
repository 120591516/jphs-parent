...
function getTotalNumber(className,totalIdName){
	var totalAuditBox = document.getElementsByClassName(className+"_red");
	var totalRegitBox = document.getElementsByClassName(className+"_blue");
	var totalAudit = 0;
	var totalRegit = 0;
	for(var i=0;i<totalAuditBox.length;i++){
		totalAudit +=parseFloat(totalAuditBox[i].innerHTML);
	}
	for(var i=0;i<totalRegitBox.length;i++){
		totalRegit +=parseFloat(totalRegitBox[i].innerHTML);
	}
	
	$("."+totalIdName+"_red").html(totalAudit);
	$("."+totalIdName+"_blue").html(totalRegit);
}