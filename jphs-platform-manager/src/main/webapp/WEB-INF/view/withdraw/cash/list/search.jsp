			...
			<tr>
				...
				<td width="200"><div class="controls col-md-6"
						...
						<select class="marage_select" id="status" name="status">
							...
							<option value="0" <c:if test="${withdrawCash.status==0}">selected="selected"</c:if>>待审核</option>
							<option value="-1" <c:if test="${withdrawCash.status==-1}">selected="selected"</c:if>>已驳回</option>
						</select>
					</div></td>
			</tr>
			<%-- <tr>
