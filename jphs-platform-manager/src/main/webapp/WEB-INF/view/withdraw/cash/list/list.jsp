			...
			<c:when test="${fn:length(list) >0}">
				<c:forEach items="${list}" var="e" varStatus="s">
					<tr class="bg_list_body" >
						...
						<td style="text-align: left;"><c:out value="${e.alipayAccount}" /></td>
						<td><fmt:formatDate value="${e.createTime}"
								pattern="yy-MM-dd HH:mm" /></td>
						<td><fmt:formatDate value="${e.auditTime}"
								pattern="yy-MM-dd HH:mm" /></td>
						<%-- <td style="text-align: center;"><c:out value="${e.remark}" /></td> --%>
						...
						<td>
							<jphs:hasPermission url="/withdraw/cash/redirectUpdate.jhtml">
								<c:if test="${e.status != -1}">
									<a onclick="redirectUpdatePage('${e.id}')" title="详情"> <img
