			...
			<c:forEach items="${list}" var="e" varStatus="s">
				<tr class="bg_list_body">
					...
					<td>
						...
						<c:if test="${user.device==1}">IOS</c:if>
						<c:if test="${user.device==2}">安卓</c:if>
						<c:if test="${user.device==3}">微信</c:if>
						<c:if test="${user.device==4}">114等注册端</c:if>
						<c:if test="${user.device==5}">PC端</c:if>
						<c:if test="${user.device==6}">WAP站</c:if>
						<c:if test="${user.device==7}">呼啦圈</c:if>
