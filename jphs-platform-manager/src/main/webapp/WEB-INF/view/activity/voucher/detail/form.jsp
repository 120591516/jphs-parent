...
<form class="form-horizontal edit_form" >
	<div class="form-group">
		...
		<div class="controls col-md-6">
			<c:if test="${voucher.productType==1 }">服务</c:if>
			<c:if test="${voucher.productType==2 }">商品</c:if>
		</div>
	</div>
	...
	<c:if test="${voucher.productType==2 }">
		<div class="form-group">
			...
			<div class="controls col-md-6">
				<c:if test="${voucher.supportType==1 }">商品</c:if>
				<c:if test="${voucher.supportType==2 }">商家</c:if>
				<c:if test="${voucher.supportType==3 }">通用</c:if>
			</div>
		</div>
	</c:if>
	<div class="form-group">
		...
		<div class="controls col-md-6">
		<c:if test="${voucher.supportType!=3 }">${voucher.productName }${voucher.goodsName }</c:if>
		
		<c:if test="${voucher.supportType==3 }">所有商品</c:if>
		</div>
	</div>
	<div class="form-group">
	...
	<c:if test="${voucher.type==1 }">
		<div class="form-group">
			...
			<div class="controls col-md-6"> 
				<fmt:formatNumber value="${voucher.amount }" type="currency"/>
				</div>
		</div>
	</c:if>
	<c:if test="${voucher.type==2 }">
		<div class="form-group">
			...
			<div class="controls col-md-6">消费满
				<fmt:formatNumber value="${voucher.conditionAmount }" type="currency"/>
			减
			<fmt:formatNumber value="${voucher.amount }" type="currency"/>
			</div>
		</div>
	...
	<c:if test="${voucher.type==3 }">
		<div class="form-group">
			...
			<div class="controls col-md-6">打${voucher.amount *10}折</div>
		</div>
	</c:if>
	...
	<button type="button" class="btn btn-info public_btn_right" data-toggle="modal" data-target="#dxModal" style="margin-right:30px;margin-bottom:20px">批量发放</button>
	<div class="marage_right_content">
		<table id="dateTable" 
			class="data_table text-center" style="width: 930px">
			<thead>
				<tr>
					<th width="40" style="padding-right:10px"><jphs:hasPermission
							url="/voucher/addUser.jhtml">
							<a onclick="showDiv('${voucher.id}',${voucher.amount })">
						...
						<c:forEach items="${list}" var="e" varStatus="s">
							<tr>
								<td><input type="checkbox" value="${e.repertoryId }"
									...
									<c:if test="${e.status ==1}">disabled="disabled"</c:if> />
								</td>
								<td>
									<%-- ${voucher.id} --%>${e.no }</td>
								...
								<td><c:if test="${e.status ==1}">已领取</c:if> <c:if
										test="${e.status ==0}">未领取</c:if></td>
...
<div class="modal fade" id="dxModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
 ...
 请输入手机号<br/>（多个手机号用英文逗号隔开）
				...
				<div class="form-group text-center">
					 <textarea rows="" cols="" name ="phnoes" id="phones" style="width:200px;">
					 </textarea>
				</div> 
