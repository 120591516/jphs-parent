...
<%@taglib prefix="jphs" uri="http://www.jinpaihushi.com/jsp/core"%>
	...
	<fieldset>
		...
		<div class="form-group">
			<!-- 文本输入 -->
			<label class="control-label col-md-4" for="input01">一级分类：</label>
			<div class="controls col-md-6">
				 ${ctp.name } 
			</div>
		</div>
		<div class="form-group">
			<!-- 文本输入 -->
			<label class="control-label col-md-4" for="input01">二级分类：</label>
			<div class="controls col-md-6">
				  ${commodityType.name }
			</div>
		</div>
		...
		<div class="form-group">
			...
			<div class="controls col-md-6">
				<input disabled="no" type="checkbox" name="quality" <c:if test="${commodity.quality==1}">checked="checked"</c:if>>正品认证
				<input disabled="no" type="checkbox" name="privacy"  <c:if test="${commodity.privacy==1}">checked="checked"</c:if> >隐私配送
				<input disabled="no" type="checkbox" name="security" <c:if test="${commodity.security==1}">checked="checked"</c:if> >药检认证
			</div>
		</div>
<%-- 
		<div class="form-group">
		...
		<div class="form-group">
			...
			<div class="controls col-md-6">
			 <input disabled="no" type="radio" name="supportVoucher" value="1" <c:if test="${commodity.supportVoucher==1}">checked="checked"</c:if>>支持
			 <input disabled="no" type="radio" name="supportVoucher" value="0" <c:if test="${commodity.supportVoucher==0}">checked="checked"</c:if>>不支持
			</div>
		</div>
